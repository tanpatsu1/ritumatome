...
def scrape_list(page) -> list:
    ...
    # 行データ抽出
    items = page.evaluate("""() => {
        const rows = document.querySelectorAll('c-r_-custom-datatable-cmp table tbody tr');
        const cell = (tr, label) => {
            const el = tr.querySelector(`[data-label="${label}"]`);
            return el ? el.innerText.trim() : "";
        };
        return Array.from(rows).map(tr => {
            const a = tr.querySelector('th[data-label="タイトル"] a');
            if (!a) return null;
            const href = a.getAttribute('href') || "";
            const m = href.match(/r-information\\/([A-Za-z0-9]+)\\//);
            return {
                id: m ? m[1] : href,
                url: href.startsWith('http') ? href : ('https://sp.ritsumei.ac.jp' + href),
                ttl: (a.getAttribute('title') || a.innerText).trim(),
                published: cell(tr, '公開日'),
                end: cell(tr, '終了日'),
                dept: cell(tr, '担当部課'),
                portal_cat: cell(tr, '配信カテゴリ'),
                deadline_raw: cell(tr, '期日設定'),
                kubun: cell(tr, 'お知らせ区分'),
            };
        }).filter(Boolean);
    }""")
...
def scrape_detail(page, url: str) -> str:
    """通知詳細ページから本文テキストを抽出(構造が不明でも動く多段フォールバック)"""
    page.goto(url, wait_until="domcontentloaded", timeout=60000)
    page.wait_for_timeout(2500)  # LWCレンダリング待ち
    body = page.evaluate("""() => {
        // 1) リッチテキスト本文(Salesforceの定番)
        const rts = Array.from(document.querySelectorAll(
            'lightning-formatted-rich-text, .slds-rich-text-editor__output'));
        let texts = rts.map(e => e.innerText.trim()).filter(t => t.length > 30);
...
{items_text}"""
    ...
    for attempt in range(3):
        try:
            ...
            print(f"  [gemini] リトライ {attempt+1}/3: {e}")
